...
The Riverscapes Command Line Interface (rscli) is a small, lightlight application for interacting with the [Riverscapes Data Exchange](https://data.riverscapes.net) and the associated [API](./api). Using rscli simplifies the process of uploading and downloading data from the Riverscapes Data Exchange. It handles authenticating your user account and validating data uploads.

# Prerequisites

You must have node version 18 installed on your computer to use rscli. You can download the latest version of node from the [official website](https://nodejs.org/). At the current time of writing, it must be version 18, preferably the Long Term Release (LTR).

# Installing rscli

Open a command line prompt (Terminal on Mac, Windows Terminal or Command Prompt on Windows) and run the following command:
