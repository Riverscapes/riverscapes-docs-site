...
As described in [Products/Reports](../products/reports), the [Riverscapes Reports Platform](https://reports.riverscapes.net) is a flexible and scalable system for summarizing riverscapes data into professional reports.
...
2. **Report properties** - metadata describing the report that are used to include it in the reporting platform user interface.
...
It may be preferable to build multiple specialized reports, rather than try to make one that has something for everyone.
...
The Riverscapes reports platform uses data produced by the [2025 run of the riverscapes network models](/initiatives/2025-CONUS-runs.mdx). More specifically, it uses our custom database of [RME metrics](https://tools.riverscapes.net/rme/data) for approximately 54,000,000 DGOs across the conterminous USA.
...
The riverscapes reports use the free and open source [Plotly Graphing Library](https://plotly.com/python/) for charting. Report authors can specify any chart types that Plotly supports.
...
A [semantic version number](https://semver.org/) (`major.minor.revision`) that is associated with each report that is generated. The version number appears inside the report and is useful for consumers to distinguish between older editions of report templates.
...
- **DGO Picker** - report consumers can pick a riverscape level path to be area of interest for their report. They can control the downstream and upstream level path extents. This excludes tributaries.
...
A few (1-4) sentences providing an overview of what the report contains. This information is used on the home page of the report platform, directly under the report graphic, and helps consumers to quick understand why they would use the report.
