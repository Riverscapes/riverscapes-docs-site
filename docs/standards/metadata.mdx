...
To ensure these are accurately described for data consumers, we can use the STRUCTURED logical type combined with a clear naming convention.

#### The structured data bridge

When documenting these types, we map three distinct representations:

1. Source JSON: The raw semi-structured data.
2. Physical Schema (Athena/SQL): The strict type definition (e.g., `STRUCT<...>`).
3. Metadata Definition: The human-readable entry in layer_definitions.json.
...
| dem_bins.geotransform    | Transformations     | NA        | STRUCTURED | ????
...
The `preferred_format` attribute is intended for the data producer to provide a suggested formatting for the values. It uses the python mini-language.
