...
![copying GUID from data exchange](/img/data-exchange/DE_Copy_ProjectID.gif)

Often, for projects that are too large (e.g. > 1.5 to 2 GB) the Data Exchange struggles zipping this file up. Because rscli does not download a zip file, but instead downloads individual files, it can be used as an alternative for downloading large projects. 
