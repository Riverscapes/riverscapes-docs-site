...
Often, for projects that are too large (e.g. > 1.5 to 2 GB) the Data Exchange strugles zipping this file up. Because the RSCLI does not download a zip file, but instead downloads individual files, it can be used as an alternative for downloading large projects. 

### Requirements
You must have read permissions to access the project before you can download it. If you can see the project details and download it in the [Data Exchange](https://data.riverscapes.net) then you should e able to download the project from RSCLI. Unlike downloading projects from the Data Exchange that will result in a zip file, rscli will download the project as raw individual uncompressed files to the specified root project directory.
