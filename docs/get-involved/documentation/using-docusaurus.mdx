...
A new page is written in [markdown](https://www.markdownguide.org/) and should have an `.mdx` extension, as that allows you to use react components described below (e.g. Youtube video, ToolsWrapper). If you don't plan on using React components then you can simply use a plain markdown file ending with `.md`. Docusaurus will convert both to web pages.
...
- `title_icon` is an optional field that places an icon to the left of the page title at top of page (and below breadcrumb navigation). Note this is different than the site level icon in the navigation bar controlled in the [`docusauras.config.ts` file](#step-6-update-site-configuration)
...
## Tabs 
Use the code syntax below to add a tab bar.  
...
<Tabs>
  <TabItem value="layer1" label="Layer 1 Details">
Type|Metrics|Description
---|---|---
Point|`Count`|*Layer 1 Description*
  </TabItem>
<TabItem value="layer2" label="Layer 2 Details">
Type|Metrics|Description
---|---|---
Line|`Length`|*Layer 2 Description*
  </TabItem>
  <TabItem value="layer3" label="Layer 3 Details">
Type|Metrics|Description
---|---|---
Polygon|`Area`|*Layer 3 Description*
    </TabItem>
</Tabs>
...
To add a detail section, use the following code syntax.

```
<details open>
<summary>*How to Use This Protocol*</summary>

This describes how to use this protocol in detail.  

- *Layers*: Points, Lines, Polygons 
- *Metrics*: Counts, Lengths, Area 
</details>
...
1. Click **Fork** on the GitHub repository page to create a copy under your own account.
2. Clone your fork locally: `git clone https://github.com/YOUR_USERNAME/REPO_NAME.git`
3. Create a descriptive branch: `git checkout -b fix/my-description`
4. Make your changes, then commit: `git add . && git commit -m "Brief description of change"`
5. Push to your fork: `git push origin fix/my-description`
6. Open a **Pull Request** from your fork's branch to the upstream `main` branch on GitHub.
7. Wait for a reviewer to approve and merge your PR. The site will rebuild automatically on merge.

#### Branching (team members with write access)

If you have write access to the repository, use a branch instead of a fork:

1. Create a branch: `git checkout -b fix/my-description`
2. Make changes, commit, and push the branch.
3. Open a Pull Request on GitHub targeting `main`.
4. Request a review if needed, then merge once approved.
